...
it('applies accepted Teams config to a real isolated OpenCode child and reads effective revision', async () => {
  ...
  try {
    ...
    const managed = createManagedConfigOwner({ agentId: 'llm-agent', executable: '/Users/fanzhang/.opencode/bin/opencode', directory: join(directory, 'opencode'),
      port, startupTimeoutMs: 5000, stopTimeoutMs: 2000, resolveCredential: async () => { throw new Error('credential resolver must not run') } })
