...
async function oneRequest<T>(socket: WssConnection, timeoutMs: number, action: () => Promise<T>): Promise<T> {
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 2_147_483_647) throw new RelayProtocolError('INVALID_INPUT', 'Console timeout invalid')
  let timer: ReturnType<typeof setTimeout> | undefined
  try {
    return await Promise.race([
      Promise.resolve().then(action),
      socket.closed.then(error => { throw error }),
      new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new RelayProtocolError('RESULT_UNKNOWN', 'Console deadline elapsed; do not replay')), timeoutMs) }),
    ])
  } finally {
    clearTimeout(timer)
    await socket.close()
...
export async function requestConsole(socket: WssConnection, input: ConsoleWireRequest, timeoutMs: number): Promise<ConsoleWireReply> {
  ...
  return oneRequest(socket, timeoutMs, async () => {
...
export async function serveConsole(socket: WssConnection, handler: (request: ConsoleWireRequest) => Promise<ConsoleWireReply>, timeoutMs: number): Promise<void> {
  return oneRequest(socket, timeoutMs, async () => {
    const request = parseConsoleWireRequest(await readText(socket))
