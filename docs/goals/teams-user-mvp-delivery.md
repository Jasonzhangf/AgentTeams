...
当前主线（本次接手复核为 `dcb3bdc`）已包含 internal.toml 真源迁移（U0/`159b78b`）及本机双 daemon 基础；
用户 CLI、完整 passive provider/receiver Work、OpenCode 多 provider、Console directory
projection 和最终 Console-offline/restart 真实收口仍未完成。runtime issue `3742b9a` 已有旧基线
candidate `d040885`，必须重新绑定当前 main 后再供 L1 CLI 使用；这不是 CLI 的重复实现范围。
